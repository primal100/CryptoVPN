...
from blockchain import blockexplorer, exchangerates, util, wallet
...
class CoinWallet:
    ...
    def get_transactions(self, period=None):
        ...
        for transaction in transactions:
            transaction.date = datetime.fromtimestamp(transaction.time)
        now = datetime.now()
        ...
        for transaction in transactions:
            ...
            transaction.total_value = sum([i.value for i in transaction.inputs], 0)
            transaction.usd_value = to_fiat_with_timestamp("USD", transaction.total_value, transaction.time, api_code=api_code)
